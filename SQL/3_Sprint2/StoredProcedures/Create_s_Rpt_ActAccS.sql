...
/****** Object:  StoredProcedure [dbo].[s_Rpt_ActAccS]    Script Date: 2017/11/27 10:41:20 ******/
SET ANSI_NULLS ON
GO
...
-- Description : Search Procedure FOR s_Rpt_ActAccS
-- ============================================= */

IF OBJECT_ID('[dbo].[s_Rpt_ActAccS]') > 0
BEGIN
	DROP PROCEDURE [dbo].[s_Rpt_ActAccS]
END
GO

CREATE PROCEDURE [dbo].[s_Rpt_ActAccS]	
	@g2sbDB nvarchar(100),
	@lastday NVARCHAR(50)
AS
BEGIN
	...
	SET @sqlStr = '
		...
		(
			SELECT TEMP.accno AS client_code,
				   max(TEMP.tdate) AS lastday FROM
			  ( SELECT accno, tdate
		...
		(
			...
			LEFT OUTER JOIN '+ @g2sbDB +'.[DBO].ae_master AS ae ON ae.aeid = clms.aeid AND ae.cmid = clms.cmid
...
--EXEC [s_Rpt_ActAccS] 'G2SB_UAT.g2sb_dev' ,'20150101'
