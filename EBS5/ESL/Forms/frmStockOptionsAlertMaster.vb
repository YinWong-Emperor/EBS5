...
Public Class FrmStockOptionsAlertMaster
    Dim cls As New ClsStockOptionsAlertMaster
    ...
    Private Sub btnCancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCancel.Click
        ...
        If Me.btnAddRecord.Enabled = False And Me.btnEdit.Enabled = False Then
            Dim dt As DataTable = TryCast(dtgOPAlert.DataSource, DataTable)
            dt.Rows.RemoveAt(dt.Rows.Count - 1)
            dt.AcceptChanges()
            EnableAdd(False)
        ElseIf Not Me.btnEdit.Enabled Then
    ...
    Private Sub FrmStockOptionsAlertMaster_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        GetMarket()
    ...
    End Sub
    Private Sub EnableAdd(ByVal blnflag As Boolean)
        Me.btnEdit.Enabled = Not blnflag
        Me.btnEnquiry.Enabled = Not blnflag
        Me.btnSave.Enabled = blnflag
        Me.txtProductCode.ReadOnly = blnflag
        Me.txtProductName.ReadOnly = blnflag
        Me.cboMarket.Enabled = Not blnflag
        Me.btnAddRecord.Enabled = Not blnflag
        Me.btnEdit.Enabled = Not blnflag
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If Me.btnEdit.Enabled = False And Me.btnAddRecord.Enabled = False Then
            ...
            Dim name As String = dtgOPAlert.Rows(rowindex).Cells("dtgProductName").Value.ToString.Trim()
            Dim market As String = dtgOPAlert.Rows(rowindex).Cells("dtgMarket").Value.ToString.Trim()
            Dim productCode As String = dtgOPAlert.Rows(rowindex).Cells("dtgProductCode").Value.ToString.Trim()
            Dim positionLimit As Object = dtgOPAlert.Rows(rowindex).Cells("dtgPositionLimit").Value
            Dim grossNet As Object = dtgOPAlert.Rows(rowindex).Cells("dtgCboGrossNet").Value
            Dim emailAlert As Object = dtgOPAlert.Rows(rowindex).Cells("dtgalert").Value
            Dim emailAlert2 As Object = dtgOPAlert.Rows(rowindex).Cells("dtgalert2").Value
            Dim monthAlert As Object = dtgOPAlert.Rows(rowindex).Cells("month_alert").Value
            Dim positionLimit2 As Object = dtgOPAlert.Rows(rowindex).Cells("dtgPositionLimit2").Value

            'selecT * from system_messages
            If cls.isRecordExist(productCode) Then
                MsgBox(GFncGetSysMsg(54))
                Return
           
            ElseIf productCode = "" Then
                MsgBox(GFncGetSysMsg(117))
                Return
            ElseIf name = "" Then
                MsgBox(GFncGetSysMsg(114))
                Return
            ElseIf market = "" Then
                MsgBox(GFncGetSysMsg(115))
                Return
            ElseIf productCode.Length > 10 Then
                MsgBox(GFncGetSysMsg(118))
                Return
            ElseIf name.Length > 60 Then
                MsgBox(GFncGetSysMsg(119))
                Return
            ElseIf market.Length > 50 Then
                MsgBox(GFncGetSysMsg(120))
                Return
            ElseIf IsDBNull(positionLimit) Then
                MsgBox(GFncGetSysMsg(116))
                Return
            Else
                If IsDBNull(grossNet) Then
                    grossNet = ""
                End If
                If IsDBNull(emailAlert) Then
                    emailAlert = ""
                End If
                If IsDBNull(emailAlert2) Then
                    emailAlert2 = ""
                End If
                If IsDBNull(monthAlert) Then
                    monthAlert = ""
                End If
                ...
                Condition = "(product_code,product_name,market,gross_net,position_limit,email_alert,month_alert,position_limit_2,email_alert_2) values(" & _
                             "'" & productCode & "','" & name & "','" & market & "'," & _
                                " '" & GetGrossNetCode(grossNet) & "', " & _
                                  " " & positionLimit & ", " & _
                               "  '" & GetAlert(emailAlert) & "', " & _
                               " '" & GetAlert(monthAlert) & "', " & _
                            " " & positionLimit2 & ", " & _
                            "  '" & GetAlert(emailAlert2) & "') "
            End If

            cls.AddRecord(Condition)
            'EnableRow(EditCode, False)
            EnableAdd(False)
            EditCode = productCode
            btnEnquiry_Click(Nothing, System.EventArgs.Empty)
            SetFocus()
        End If
    ...
    Private Function GetAlert(ByVal val As String) As Integer
        ...
        End Select
    End Function
    Private Function GetAlert(ByVal val As Object) As Integer
        Select Case val
            Case "True"
                Return 1
            Case "False"
    ...
    'Private Sub btnErrEnq_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    'Private Sub DtgErrorReport_Sorted(ByVal sender As Object, ByVal e As System.EventArgs)
    '    ErrorColor()
    'End Sub
    ...
    Private Sub btnAddRecord_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAddRecord.Click
        ...
        dt.Rows.Add()
        dt.AcceptChanges()
        For row As Integer = 0 To dtgOPAlert.RowCount - 2
            For col As Integer = 0 To dtgOPAlert.ColumnCount - 1
                Me.dtgOPAlert.Rows(row).Cells(col).ReadOnly = True
            Next
        Next
        For col As Integer = 0 To dtgOPAlert.ColumnCount - 1
            Me.dtgOPAlert.Rows(dt.Rows.Count - 1).Cells(col).ReadOnly = False
            Me.dtgOPAlert.Rows(dt.Rows.Count - 1).Cells(col).Style.BackColor = Color.White
            Me.dtgOPAlert.Rows(dt.Rows.Count - 1).Cells(col).Style.SelectionBackColor = Color.White
            Me.dtgOPAlert.Rows(dt.Rows.Count - 1).Cells(col).Style.SelectionForeColor = Color.Black
        Next
