...
Public Class FrmImportDataUS
    ...
    Public currUSDate As Date = Nothing
    Public nextUSDate As Date = Nothing
    ...
    Private Sub SetProgress()
        ...
        If InvokeRequired Then
            ...
            SyncLock progressThreadLock
                importProgressHistories = cls.FncGetProgressUS(pImportCurrentGroup, pImportStepId)
            End SyncLock
            Me.Invoke(setPgs)
        Else
...
#Region "事件"
    ...
    Public Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If btnSave.Enabled Then
            GSubWriteEventLog(String.Format("{0} -> Import data (US), CurrentTradeDate:{1}, NextTradeDate:{2}", Format(Now, "yyyy-MM-dd HH:mm:ss"), lblCurrentTradeDate.Text, lblNextTradeDate.Text), GStrEPath)
    ...
    Private Sub btnCancel_Click(sender As Object, e As EventArgs) Handles btnCancel.Click
        If Not peFormStatus = EnumFormStatus.Loading Then
            Me.Close()
        Else
            If GSubShowYNConfirm("Do you confirm to cancel the import data (US)?") = Windows.Forms.DialogResult.Yes Then
                CancelImport()
...
#End Region
    ...
    Private Sub InitData()
        ...
        nextUSDate = cls.FncGetG2BLastTradeDateUS()
        currUSDate = GFncGetTDateUS()
        ...
        Me.lblCurrentUSTradeDate.Text = Format(currUSDate, "dd/MM/yyyy")
        Me.lblNextUSTradeDate.Text = Format(nextUSDate, "dd/MM/yyyy")

        '-------------------------------------
        ' Need to modify the date checking.
        '-------------------------------------
        ...
        If data.Rows.Count > 0 Then
            ...
            If Not procflag Then
                Dim groupTB As Object = cls.FncGetExecutingGroupUS()
            ...
            ElseIf tradeDate < g2bDate Then
                txtMessage.Text = String.Format("Not yet imported {0} data!", Format(g2bDate, "dd/MM/yyyy"))
                GSubWriteEventLog(String.Format("Not yet imported {0} data!", Format(g2bDate, "dd/MM/yyyy")), GStrEPath)
                txtMessage.ReadOnly = False
                txtMessage.Enabled = False
                txtMessage.ForeColor = Color.Red
                SetStatus(EnumFormStatus.Invalid)
            ElseIf g2bDate <> nextUSDate Then
                txtMessage.Text = String.Format("Next US Trade Date should same as Next Trade Date: {0} !", Format(g2bDate, "dd/MM/yyyy"))
                GSubWriteEventLog(String.Format("Next US Trade Date should same as Next Trade Date: {0} !", Format(g2bDate, "dd/MM/yyyy")), GStrEPath)
                txtMessage.ReadOnly = False
                txtMessage.Enabled = False
                txtMessage.ForeColor = Color.Red
                SetStatus(EnumFormStatus.Invalid)
            ElseIf currUSDate >= tradeDate Then
    ...
    Private Sub DoImport()
        ...
        Try
            ...
            result = cls.FncImportDataUS(pImportCurrentGroup, g2bDate)
        Finally
            pImportIsDone = True
        End Try
        ...
        If Not result Is Nothing Then
            ...
            mySendEmail(EmailFrom, EmailTo, EmailSubjectPrefix & " - [FAIL] - Liq. Data (US) Imported " & Format(g2bDate, "dd/MM/yyyy"), "Import Error: " & result, GStrEIP)
            Return
        Else
            ...
            mySendEmail(EmailFrom, EmailTo, EmailSubjectPrefix & " - [SUCCESS] - Liq. Data (US) Imported " & Format(g2bDate, "dd/MM/yyyy"), "Liq. Data (US) Imported sucessfully ", GStrEIP)
        End If
        DoImportIsDone = True
    End Sub
