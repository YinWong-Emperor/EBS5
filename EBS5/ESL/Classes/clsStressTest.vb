...
Public Class clsStressTest
    ...
    Protected Friend Function FncLoadCombo() As DataSet
        Dim str As String = "select distinct stk_code from STPortfolio order by stk_code"
    ...
    Protected Friend Function FncLoadDataSet() As DataSet
        Dim str As String = "select b.stk_code, sum(b.net_market_value) as market, sum(b.net_market_value) as market_value, " & _
                            "sum(b.net_qty) as qty from STPortfolio b join STCLTMASTER a on b.CLT_CODE = a.clt_code " & _
                            "where a.CLT_TYPE = 'M' group by b.stk_code order by sum(b.net_market_value) DESC"
        Return GFncRtnDS(GSCnLiqConn, str)
    End Function
    Protected Friend Function FncLoadMV() As Double
        Dim str As String = "select sum(b.net_market_value) as market from STPortfolio b join STCLTMASTER a on b.CLT_CODE = a.clt_code " & _
                                   "where a.CLT_TYPE = 'M'"
    ...
    Protected Friend Function FncLoadExistingDataSet(ByVal txnDate As String) As DataSet
        ...
        '                    "from STPortfolio b join STCLTMASTER a on b.CLT_CODE = a.clt_code " & _
    ...
    Protected Friend Function FncGetClient(ByVal FRR As Boolean) As DataTable
        ...
        '                    "join STPortfolio b on a.CLT_CODE = b.clt_code " & _
        '                    "join STCLTMASTER c on a.CLT_CODE = c.clt_code where b.net_market_value > 0 and c.CLT_TYPE = 'M' order by a.CLT_CODE "
        'Dim str As String = "select a.CLT_CODE as client_code, a.DR_BAL, a.CLT_NAME as name, b.stk_code as Stk_code, " & _
        ...
        '        "from  STCLTMASTER c left join STPortfolio b on c.CLT_CODE = b.clt_code where c.CLT_TYPE = 'M' and c.CR_bal <= 0 order by c.CLT_CODE "
        'Else
        str = "select c.CLT_CODE as client_code, c.DR_BAL-c.interest as DR_BAL, c.CLT_NAME as name, b.stk_code as Stk_code, round(b.net_qty, 0) as qty, " & _
            ...
            "0.00 as short from  STCLTMASTER c left join (select * from STPortfolio where stk_code not in (select distinct stkno from STSUSPENDSTOCK)) b " & _
    ...
    Protected Friend Function FncGetStock() As DataTable
        Dim str As String = "select distinct b.Stk_code, 0.00 as price, 0.00 as new_price from STPortfolio b " & _
                            "inner join STCLTMASTER a on b.CLT_CODE = a.clt_code " & _
    ...
    Protected Friend Function FncExpertExcel(ByVal dgvSingle As DataGridView, ByVal dgvSign As DataGridView, ByVal indate As Date, ByVal LB As Boolean, ByVal cSF As Double, ByVal cHC As Double, Optional ByVal cut As Double = 1) As Boolean
        ...
        Dim str As String = "select a.clt_code, a.clt_name, a.run_code, b.run_name, 0.00 as concentration, " & _
                            ...
                            "from stcltmaster a left join staemaster b on a.run_code = b.run_code " & _
                            "left join stportfolio c on a.clt_code = c.clt_code " & _
        ...
        str = "select a.clt_code, sum(b.net_market_value) as nmv from stcltmaster a left join stportfolio b on " & _
