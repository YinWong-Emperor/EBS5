...
Public Class ClsRptDebitBalConUS
    ...
    Protected Friend Function lFncPrepareMC(ByVal lstrQuery As String, ByVal lstrSort As String, ByVal MC As String) As DataTable
        ...
        '        " cast(0 as decimal(18,2)) as top1_market_value, cast(0 as decimal(18,2)) as top1_stock_qty, '' as top1_stock_code, " & _
        '        " cast(0 as decimal(18,2)) as top2_market_value, cast(0 as decimal(18,2)) as top2_stock_qty, '' as top2_stock_code, " & _
        '        " cast(0 as decimal(18,2)) as top3_market_value, cast(0 as decimal(18,2)) as top3_stock_qty, '' as top3_stock_code, " & _
        '        " run_name, cast(0 as decimal(18,2)) as margin_call_amount,  " & _
        '        " '' as Due_Undue, '' as Risk " & _
        '        " FROM stcltmaster_US mst LEFT JOIN  stcltliq_US liq ON  mst.clt_code = liq.clt_code " & _
        ...
        lstrSQL = " select * from STPortfolio_US " & _
                " where clt_code in (select mst.clt_code from stcltmaster_US mst " & _
                " LEFT JOIN  stcltliq_US liq ON  mst.clt_code = liq.clt_code " & _
                 lstrQuery & ")  "
        ...
        For Each ldtwTemp As DataRow In ldtsTemp.Tables(0).Rows
            ...
            'Mail from Rick Wong 2018/3/7 (周三) 9:56 FW: Adding addition information for EBS4 report: added top3 qty
            ...
            For lintCnt As Integer = 0 To ldtaTop.Length - 1
                Select Case lintCnt
                    Case 0
                        ...
                        ldtwTemp.Item("top1_stock_qty") = ldtaTop(0).Item("net_qty")
                    ...
                    Case 1
                        ...
                        ldtwTemp.Item("top2_stock_qty") = ldtaTop(1).Item("net_qty")
                    ...
                    Case 2
                        ...
                        ldtwTemp.Item("top3_stock_qty") = ldtaTop(2).Item("net_qty")
    ...
    ByVal strTitle As String, ByVal MC As String) As ReportClass
        Dim rpt As New RptDebitBalConUS
        ...
        If ldtsTemp.Rows.Count > 0 Then
            ...
            Return rpt
        Else
            Return clsRpt.lfncRtnEmptyRpt("Debit Balance Concentration Report - (Margin, Cash & Finance) " & strTitle)
        End If
