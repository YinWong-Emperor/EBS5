...
Public Class ClsRptCommModify
    ...
    Protected Friend Function PrintCommRpt(ByVal inMonth As String) As ReportClass
        ...
        Dim rpt As New RptCommModifyDiff

        query = "select a.oid, a.aeno as Aae, isnull(b.aeno,'') as ae,  isnull(b.txmonth, a.txmonth) as txmonth, a.acno as Aaccno, isnull(b.accno,'') as accno, a.qty as Aqty, isnull(b.qty,0) as qty," & _
                    ...
                    " a.comm_rate as Acomm_rate, isnull(b.comm_rate,0) as comm_rate, case when a.tradetype=4 then 'I-trade' else 'Normal' end Atradetype, isnull(b.tradetype,'') as  tradetype, case when a.adjaction='A' then 'Add' when a.adjaction='M' then 'Modify' when a.adjaction='D' then 'Deleted' end adjaction " & _
                    " from comm_adj_s a left outer join view_comm_trade_s b on a.oid=b.oid where (a.adjaction='M' or a.adjaction='A' or a.adjaction ='D') and a.txmonth='" & inMonth & "' order by accno asc, b.tdate asc"
    ...
    Protected Friend Function PrintFutRpt(ByVal inMonth As String) As ReportClass
        Dim query As String
        Dim ldtsTemp As DataSet
        Dim rpt As New RptCommMoidify_fut
