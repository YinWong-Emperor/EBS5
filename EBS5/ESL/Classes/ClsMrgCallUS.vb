...
Public Class ClsMrgCallUS
    ...
    Protected Friend Function lFncPrepareMC(ByVal lstrQuery As String, ByVal lstrSort As String, ByVal MC As String) As DataTable
        ...
        '        " run_name, cast(0 as decimal(18,2)) as margin_call_amount,  " & _
        '        " '' as Due_Undue, '' as Risk " & _
        '        " FROM #stcltmaster_mrg_US mst inner JOIN #clt_tmp_mrg_US liq ON  mst.clt_code = liq.clt_code " & _
        '        " left outer join staemaster ae on ae.run_code = mst.run_code WHERE 1=1 "
        ...
        lstrSQL = " select * from STPortfolio_US " & _
                " where clt_code in (select mst.clt_code from #stcltmaster_mrg_US mst " & _
                " inner JOIN  #clt_tmp_mrg_US liq ON  mst.clt_code = liq.clt_code " & _
                lstrQuery & ")  "
    ...
    Protected Friend Function lFncPrepareMC2(ByVal lstrQuery As String, ByVal lstrSort As String, ByVal MC As String) As DataTable
        ...
        lstrSQL = "SELECT  *, mst.os_day as liq_day, " & _
                ...
                " run_name, cast(0 as decimal(18,2)) as margin_call_amount,  " & _
                " '' as Due_Undue, '' as Risk " & _
                " FROM #stcltmaster_mrg_US2 mst inner JOIN #clt_tmp_mrg_US2 liq ON  mst.clt_code = liq.clt_code " & _
                " left outer join staemaster ae on ae.run_code = mst.run_code WHERE 1=1 "
        lstrSQL += lstrQuery & " order by " & lstrSort
        ...
        lstrSQL = " select * from STPortfolio_US " & _
                " where clt_code in (select mst.clt_code from #stcltmaster_mrg_US2 mst " & _
                " inner JOIN  #clt_tmp_mrg_US2 liq ON  mst.clt_code = liq.clt_code " & _
                lstrQuery & ")  "
        Dim ldtTop As DataTable = GFncRtnDS(GSCnLiqConn, lstrSQL).Tables(0)
        For Each ldtwTemp As DataRow In ldtsTemp.Tables(0).Rows
            ldtwTemp.Item("top1_stock_code") = ""
    ...
    ByVal strTitle As String, ByVal MC As String) As ReportClass
        Dim rpt As New RptMarginCallUS
...
ByVal strTitle As String, ByVal MC As String) As ReportClass
    ...
    Protected Friend Sub lFncCreateList()
        ...
        Try
            ...
            lstrSQL = "select * into #stcltmaster_mrg_US from client_liq_master_US"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "update #stcltmaster_mrg_US " & _
                        "set #stcltmaster_mrg_US.os_day = 1 "
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "update #stcltmaster_mrg_US " & _
                        "set #stcltmaster_mrg_US.os_day = stcltliq_US.liq_day " & _
                        "from stcltliq_US " & _
                        "where #stcltmaster_mrg_US.clt_code = stcltliq_US.clt_code"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)
            ...
            lstrSQL = "select a.clt_code, a.run_code, a.mc_dr_bal as dr, a.mc_act_ratio as ar, " & _
                        ...
                        "from #stcltmaster_mrg_US a left join client_mkt_mrg b on a.clt_code = b.client_code " & _
                        "where (a.clt_type = 'M' or a.clt_type = 'F')"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            'lstrSQL = "select clt_code " & _
            '            "into #clt_tmp_mrg_US " & _
            ...
            lstrSQL = "select a.accno, sum(a.amount) as amount " & _
                        ...
                        "from client_fund_movement a, #stcltmaster_mrg_US b " & _
                        "where a.accno = b.clt_code and b.clt_type = 'C' " & _
                        "group by a.accno"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "update #stcltmaster_mrg_US set deposit = 0"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "update #stcltmaster_mrg_US " & _
                        "set #stcltmaster_mrg_US.deposit = #cashin_tmp_mrg.amount " & _
                        "from #cashin_tmp_mrg " & _
                        "where #stcltmaster_mrg_US.clt_code = #cashin_tmp_mrg.accno " & _
                        "and #cashin_tmp_mrg.amount <> 0"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "select clt_code " & _
                        "into #os_tmp_mrg " & _
                        "from #stcltmaster_mrg_US " & _
                        "where clt_type = 'C' " & _
                        "and ((withdrawal > 0 and withdrawal > deposit) or (mc_act_ratio > 0.7) or (short = 1)) "
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "insert into #clt_tmp_mrg_US(clt_code) select clt_code from #os_tmp_mrg "
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)
    ...
    Protected Friend Sub lFncCleanTempTable()
        ...
        Try
            ...
            lstrSQL = "drop table #stcltmaster_mrg_US"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "drop table #clt_tmp_mrg_US "
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)
    ...
    Protected Friend Sub lFncCreateList2()
        ...
        Try
            ...
            lstrSQL = "select * into #stcltmaster_mrg_US2 from client_liq_master_US"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "update #stcltmaster_mrg_US2 " & _
                        "set #stcltmaster_mrg_US2.os_day = 1 "
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "update #stcltmaster_mrg_US2 " & _
                        "set #stcltmaster_mrg_US2.os_day = stcltliq_US.liq_day " & _
                        "from stcltliq_US " & _
                        "where #stcltmaster_mrg_US2.clt_code = stcltliq_US.clt_code"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)
            ...
            lstrSQL = "select a.clt_code, a.run_code, a.mc_dr_bal as dr, a.mc_act_ratio as ar, " & _
                        ...
                        "isnull(b.market_value, 0) as undue_mv, a.short " & _
                        "into #margin_tmp_mrg " & _
                        "from #stcltmaster_mrg_US2 a left join client_mkt_mrg b on a.clt_code = b.client_code " & _
                        "where (a.clt_type = 'M' or a.clt_type = 'F')"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "select clt_code " & _
                        "into #clt_tmp_mrg_US2 " & _
                        "from #margin_tmp_mrg " & _
            ...
            lstrSQL = "select a.accno, sum(a.amount) as amount " & _
                        "into #cashin_tmp_mrg " & _
                        "from client_fund_movement a, #stcltmaster_mrg_US2 b " & _
                        "where a.accno = b.clt_code and b.clt_type = 'C' " & _
                        "group by a.accno"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "update #stcltmaster_mrg_US2 set deposit = 0"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "update #stcltmaster_mrg_US2 " & _
                        "set #stcltmaster_mrg_US2.deposit = #cashin_tmp_mrg.amount " & _
                        "from #cashin_tmp_mrg " & _
                        "where #stcltmaster_mrg_US2.clt_code = #cashin_tmp_mrg.accno " & _
                        "and #cashin_tmp_mrg.amount <> 0"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "select clt_code " & _
                        "into #os_tmp_mrg " & _
                        "from #stcltmaster_mrg_US2 " & _
                        "where clt_type = 'C' " & _
                        "and ((withdrawal > 0 and withdrawal > deposit) or (mc_act_ratio > 0.7) or (short = 1)) "
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "insert into #clt_tmp_mrg_US2(clt_code) select clt_code from #os_tmp_mrg "
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)
    ...
    Protected Friend Sub lFncCleanTempTable2()
        ...
        Try
            ...
            lstrSQL = "drop table #stcltmaster_mrg_US2"
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)

            lstrSQL = "drop table #clt_tmp_mrg_US2 "
            GFncRunSQL(GSCnLiqConn, MyTrans, lstrSQL, 0)
