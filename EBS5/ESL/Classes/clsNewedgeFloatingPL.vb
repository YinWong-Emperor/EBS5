...
Public Class clsNewedgeFloatingPL
    ...
    Public Function FncGenReport(ByVal tdate As Date, ByVal pCounterParty As String) As ReportClass
        ...
        Dim rpt As New rptNewedgeFloatingPL
        ...
        End If
        str = "select sum(PL) as PL from vw_liq_header where tdate >= '" & Format(openDate, "yyyy/MM/dd") & "' and tdate < '" & Format(tdate, "yyyy/MM/dd") & "' and counterparty = '" & pCounterParty & "'"
        ...
        str = "select sum(PL) as pl from FuturesClosePost where sysdate >= '" & Format(openDate, "yyyy/MM/dd") & _
            "' and counterparty = '" & pCounterParty & "' and sysdate < '" & Format(tdate, "yyyy/MM/dd") & "'"
        balDt = GFncRtnDS(GSCnSqlConn, str).Tables(0)
        If balDt.Rows.Count > 0 Then
        ...
        str = "select sum(PL) as PL from vw_liq_header where tdate = '" & Format(tdate, "yyyy/MM/dd") & "' and counterparty = '" & pCounterParty & "'"
        balDt = GFncRtnDS(GSCnSqlConn, str).Tables(0)
        If balDt.Rows.Count > 0 Then
            'nClsBal += GFncNoNullValue(balDt.Rows(0).Item("PL")) + nOpnBal
            nClsBal += GFncNoNullValue(balDt.Rows(0).Item("PL"))
        End If
        str = "select sum(PL) as pl from FuturesClosePost where counterparty = '" & pCounterParty & "' and sysdate = '" & Format(tdate, "yyyy/MM/dd") & "'"
        balDt = GFncRtnDS(GSCnSqlConn, str).Tables(0)
        If balDt.Rows.Count > 0 Then
        ...
        str = " select * from opening_balance_adj " & _
                ...
                " and adjtdate <= '" & Format(tdate, "yyyy/MM/dd") & "' " & _
        ...
        If ldtAdjOpnBal.Rows.Count > 0 Then
            For Each ldrAdjOpnBal As DataRow In ldtAdjOpnBal.Rows
                If ldrAdjOpnBal("Adjtdate") = tdate Then
        ...
        str = "(select 'O' as [type], a.code as pcode, b.product_name as product, " & _
            ...
            "else (a.price - a.closing_price) * a.contract_size * a.qty end as floatingPL, 'F' as used, a.strike, CASE WHEN a.callput ='C' THEN 'Call' WHEN a.callput='P' THEN 'Put' ELSE '' END as callput, '' AS mapped  " & _
            "from futuresOP a left join futures_product_master b on b.product_code = a.code " & _
            "where a.counterparty = '" & pCounterParty & "' and a.sysdate = '" & Format(tdate, "yyyy/MM/dd") & "') union all " & _
            ...
            "'' as MDFlag, a.accno as ID, a.pl as floatingPL, 'F' as used, a.strike, CASE WHEN a.callput ='C' THEN 'Call' WHEN a.callput='P' THEN 'Put' ELSE '' END as callput,'' AS mapped  " & _
            "from FuturesClosePost a left join futures_product_master b on b.product_code = a.code " & _
            "where a.counterparty = '" & pCounterParty & "' and a.sysdate = '" & Format(tdate, "yyyy/MM/dd") & "') order by [type] DESC, pcode, tdate, sday, strike, callput"
        ...
        str = "(select 'O' as [type], b.d_code as pcode, a.product, a.buy, a.sell, a.tdate, " & _
           "day(a.settle_date) as sday, a.monthcode, a.contract_size as csize, a.price, a.closing_price as cprice, 0.000000 as lprice, " & _
           "a.monthly_daily as MDFlag, cast(a.noid as nvarchar(20)) as ID, a.floating as floatingPL, 'F' as used, " & _
           " cast(case when isadjusted='Y' then 1 else 0 end as bit) as isadjusted, a.strike, CASE WHEN a.callput ='C' THEN 'Call' WHEN a.callput='P' THEN 'Put' ELSE '' END as callput " & _
           "from vw_cap_OP a left join product_mapping b on b.d_newedge_code = a.product and a.counterparty = b.d_counterparty " & _
           " where a.tdate = '" & Format(tdate, "yyyy/MM/dd") & "' and a.counterparty = '" & pCounterParty & "' AND b.d_isoption=1 AND a.callput <>'') union all " & _
           "(select 'L' as [type], b.d_code as pcode, a.product, a.buy, a.sell, a.tdate, day(a.settle_Date) as sday, a.monthcode, " & _
           "a.contract_size as csize, a.price as price, 0.000000 as cprice, a.liq_price as lprice, a.monthly_daily as MDFlag, " & _
           ...
           "b.d_newedge_code = a.product and  a.counterparty = b.d_counterparty where a.tdate = '" & _
           Format(tdate, "yyyy/MM/dd") & "' and a.counterparty = '" & pCounterParty & "' AND b.d_isoption=1 AND a.callput <>'') union all " & _
           "(select 'L' as [type], b.d_code as pcode, a.product, 0 as buy, 0 as sell, a.tdate, day(a.settle_date) as sday, a.monthcode, " & _
           "a.contract_size as csize, 0.000000 as price, 0.000000 as cprice, 0.000000 as lprice, a.monthly_daily as MDFlag, " & _
           "cast(a.nid as nvarchar(20)) as ID, a.PL as floatingPL, 'F' as used,  " & _
           " cast(case when isadjusted='Y' then 1 else 0 end as bit) as isadjusted, a.strike, CASE WHEN a.callput ='C' THEN 'Call' WHEN a.callput='P' THEN 'Put' ELSE '' END as callput " & _
           " from vw_liq_header a left join product_mapping b on " & _
           "b.d_newedge_code = a.product  and  a.counterparty = b.d_counterparty where a.tdate = '" & Format(tdate, "yyyy/MM/dd") & "' and a.counterparty = '" & pCounterParty & "' AND b.d_isoption=1 AND a.callput <>'') " & _
            "UNION ALL" & _
           ...
           " cast(case when isadjusted='Y' then 1 else 0 end as bit) as isadjusted, a.strike, CASE WHEN a.callput ='C' THEN 'Call' WHEN a.callput='P' THEN 'Put' ELSE '' END as callput " & _
           "from vw_cap_OP a left join product_mapping b on b.d_newedge_code = a.product and a.counterparty = b.d_counterparty " & _
           " where a.tdate = '" & Format(tdate, "yyyy/MM/dd") & "' and a.counterparty = '" & pCounterParty & "' AND b.d_isoption = 0 AND a.callput = '') union all " & _
           "(select 'L' as [type], b.d_code as pcode, a.product, a.buy, a.sell, a.tdate, day(a.settle_Date) as sday, a.monthcode, " & _
           "a.contract_size as csize, a.price as price, 0.000000 as cprice, a.liq_price as lprice, a.monthly_daily as MDFlag, " & _
           "cast(a.noid as nvarchar(20)) as ID, 0.000000 as floatingPL, 'T' as used,  " & _
           " cast(case when isadjusted='Y' then 1 else 0 end as bit) as isadjusted, a.strike, CASE WHEN a.callput ='C' THEN 'Call' WHEN a.callput='P' THEN 'Put' ELSE '' END as callput " & _
           " from vw_cap_CP a left join product_mapping b on " & _
           "b.d_newedge_code = a.product and  a.counterparty = b.d_counterparty where a.tdate = '" & _
           Format(tdate, "yyyy/MM/dd") & "' and a.counterparty = '" & pCounterParty & "' AND b.d_isoption = 0 AND a.callput = '') union all " & _
           "(select 'L' as [type], b.d_code as pcode, a.product, 0 as buy, 0 as sell, a.tdate, day(a.settle_date) as sday, a.monthcode, " & _
           "a.contract_size as csize, 0.000000 as price, 0.000000 as cprice, 0.000000 as lprice, a.monthly_daily as MDFlag, " & _
           "cast(a.nid as nvarchar(20)) as ID, a.PL as floatingPL, 'F' as used,  " & _
           " cast(case when isadjusted='Y' then 1 else 0 end as bit) as isadjusted, a.strike, CASE WHEN a.callput ='C' THEN 'Call' WHEN a.callput='P' THEN 'Put' ELSE '' END as callput " & _
           " from vw_liq_header a left join product_mapping b on " & _
           "b.d_newedge_code = a.product  and  a.counterparty = b.d_counterparty where a.tdate = '" & Format(tdate, "yyyy/MM/dd") & "' and a.counterparty = '" & pCounterParty & "' AND b.d_isoption=0 AND a.callput ='') order by [type] DESC, pcode, tdate, sday, strike, callput"
        ...
        For Each edr As DataRow In eDt.Rows
            ...
            cprice = GFncNoNullValue(edr("cprice"))
            newedgeDr = nDt.Select("pcode = '" & pCode & "' and monthcode = '" & mCode & "' and MDFlag = 'D' and sDay = " & sDate & " and (strike = " & pStrike & " OR strike*csize  = " & pStrike * edr("csize") & ") and callput = '" & pCallPut & "'", "")
            If newedgeDr.Length > 0 Then
                edr("MDFlag") = "D"
            Else
                edr("MDFlag") = "M" 'Might be wrong for not-matched products
                newedgeDr = nDt.Select("pcode = '" & pCode & "' and monthcode = '" & mCode & "' and MDFlag = 'M' and (strike = " & pStrike & " OR strike*csize  = " & pStrike * edr("csize") & ") and callput = '" & pCallPut & "'", "")
            ...
            If type = "O" Then 'And cprice = 0 Then
                newedgeDr = nDt.Select("pcode = '" & pCode & "' and monthcode = '" & mCode & "' and MDFlag = 'D' and sDay = " & sDate & " and (strike = " & pStrike & " OR strike*csize  = " & pStrike * edr("csize") & ") and callput = '" & pCallPut & "'", "")
                If newedgeDr.Length > 0 Then
        ...
        Dim ldtCPAdj As DataTable = lcCls.GenCPAdjReport(tdate, pCounterParty)
        rpt.Subreports("rptFuturesStatementOPAdj.rpt").SetDataSource(ldtOPAdj)
        rpt.Subreports("rptFuturesStatementCPAdj.rpt").SetDataSource(ldtCPAdj)

        rpt.Subreports("rptNewedgePloatingPLSubG2BF").SetDataSource(eDt)
        rpt.Subreports("rptNewedgePloatingPLSubNewedge").SetDataSource(nDt)
        rpt.SetParameterValue("user", GStrloginID)
        rpt.SetParameterValue("eOpnBal", eOpnBal)
        rpt.SetParameterValue("nOpnBal", nOpnBal)
        rpt.SetParameterValue("eClsBal", eClsBal)
        rpt.SetParameterValue("nClsBal", nClsBal)
        rpt.SetParameterValue("ePL", ePL)
        rpt.SetParameterValue("nPL", nPL)
        rpt.SetParameterValue("eFloating", eFloating)
        rpt.SetParameterValue("nFloating", nFloating)
        rpt.SetParameterValue("CounterParty", pCounterParty)
        rpt.SetParameterValue("NoofOPAdj", ldtOPAdj.Rows.Count)
        rpt.SetParameterValue("NoofCPAdj", ldtCPAdj.Rows.Count)
        rpt.SetParameterValue("AdjeOpnBal", eAdjOpnBal)
        rpt.SetParameterValue("AdjnOpnBal", nAdjOpnBal)
        rpt.SetParameterValue("AdjRemark", lstrAdjRemark)
        Return rpt
    End Function

    Private Sub InsertClosingPrice(ByRef edr As DataRow, ByVal nDr As DataRow)
        If GFncNoNullValue(nDr("csize")) <> 0 And GFncNoNullValue(nDr("cprice")) <> 0 Then
            edr("cprice") = GFncNoNullValue(nDr("cprice")) * GFncNoNullValue(nDr("csize")) / GFncNoNullValue(edr("csize"))
            CalFloating(edr)
        Else
            If (GFncNoNullValue(nDr("cprice") <> 0)) Then
    ...
    Private Sub InsertDr(ByRef dt As DataTable, ByRef sdr As DataRow, ByVal sDt As DataTable, ByVal flag As String)
        ...
        Dim pStrike As Decimal = sdr("strike")
        ...
        Else
            ...
            Else
                ...
                Else
                    ...
                    If sourceDr.Length > 0 Then
                        dr("product") = GFncNoNullString(sourceDr(0).Item("product")).Trim + "<Not Mapped>"
                    Else
                        dr("product") = "<Not Mapped>"
                    End If
                End If
                'Fix bug 2013/12/31 Andrew
