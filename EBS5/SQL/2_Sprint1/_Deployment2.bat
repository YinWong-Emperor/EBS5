...
@REM -------开始预生成内容-----
@echo SET NOCOUNT ON > .\_DeployListScripts_liq.sql
@echo GO >> .\_DeployListScripts_liq.sql
@REM -------#4存储过程-----
@call:LOOP_DEAL_SQL_FILES_FROM_DIR .\ESL_Liq_SP


@REM -------#9结尾---------
@echo PRINT ''>> .\_DeployListScripts_liq.sql
@echo PRINT 'All Script Execute Successfully!'>> .\_DeployListScripts_liq.sql


@echo run scripts...
@echo.
@REM -------开始执行脚本-----
@SQLCMD -S %DB_IP% -U %DB_USER% -P %DB_PWD% -d %DB_Name% -i .\_DeployListScripts_liq.sql -b >> %LOG%
...
@echo:>> .\_DeployListScripts_liq.sql
@echo:>> .\_DeployListScripts_liq.sql
@echo PRINT '-------- Executing %TMP_NAME% --------'>> .\_DeployListScripts_liq.sql

@FOR /r %~1 %%i in (*.sql) DO @(
echo PRINT '%~1\%%~nxi' >> .\_DeployListScripts_liq.sql
echo :r "%~1\%%~nxi">> .\_DeployListScripts_liq.sql
echo:>> .\_DeployListScripts_liq.sql
)
@GOTO:EOF  
