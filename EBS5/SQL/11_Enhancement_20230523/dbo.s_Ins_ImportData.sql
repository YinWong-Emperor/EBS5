...
/****** Object:  StoredProcedure [dbo].[s_Ins_ImportData]    Script Date: 2023/5/23 下午 06:25:13 ******/
SET ANSI_NULLS ON
GO
...
'
 SET @sql3 = N'	IF OBJECT_ID(N''TEMPDB..#temp_stcltportfolio_view'',N''U'') IS NOT NULL					DROP TABLE  #temp_stcltportfolio_view;
	...
	IF OBJECT_ID(N''TEMPDB..#client_last_trade_day'',N''U'') IS NOT NULL			DROP TABLE  #client_last_trade_day;
	IF OBJECT_ID(N''TEMPDB..#cltTotalTurnover'',N''U'') IS NOT NULL			DROP TABLE  #cltTotalTurnover;
'
...
'
 SET @sql9 = N'		) AS sum_qty,
	...
	-- import client last trade date
	select client_code, lastday into #client_last_trade_day from ' + @G2BSDB + N'.dbo.view_IT_client_last_trade_day; 
 ...
 SET @sql13 = N'					AND (
	...
	BEGIN
		...
		DELETE FROM exchangerate WHERE system_type =''Futures'' AND tdate = @fdbdate;
		  ...
		  INSERT INTO exchangerate
		  SELECT ''Securities'' AS system_type, currency AS currency_in, ''HKD'' AS currency_out, rate/base AS ex_rate, @dt_now AS lupdtdate, @sdbdate AS tdate
...
'
 SET @sql29 = N'	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	--Update Client Turnover
	update ' + @LiqDB + N'.dbo.STCLTMASTER set STCLTMASTER.lastTradeDate = #client_last_trade_day.lastday from #client_last_trade_day where STCLTMASTER.CLT_CODE = #client_last_trade_day.client_code;
	select client_code, sum(total) as turnover into #cltTotalTurnover from #temp_ststktxnlst_view group by client_code;
	update ' + @LiqDB + N'.dbo.stcltmaster set prev_year_turnover = curr_year_turnover, curr_year_turnover = 0 where YEAR(@LastDate)<>YEAR(@tradeDate);
	update ' + @LiqDB + N'.dbo.stcltmaster set prev_mth_turnover = curr_mth_turnover, curr_mth_turnover = 0 where MONTH(@LastDate)<>MONTH(@tradeDate);
	update ' + @LiqDB + N'.dbo.stcltmaster set curr_mth_turnover = curr_mth_turnover + #cltTotalTurnover.turnover from #cltTotalTurnover where clt_code = #cltTotalTurnover.client_code;
	update ' + @LiqDB + N'.dbo.stcltmaster set curr_year_turnover = curr_year_turnover + #cltTotalTurnover.turnover from #cltTotalTurnover where clt_code = #cltTotalTurnover.client_code; 
