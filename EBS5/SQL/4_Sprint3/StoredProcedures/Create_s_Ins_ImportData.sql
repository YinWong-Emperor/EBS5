...
IF  EXISTS (SELECT 1
	FROM sys.objects
	WHERE object_id = OBJECT_ID(N'[dbo].[s_Ins_ImportData]') AND TYPE IN (N'P', N'PC'))
	DROP PROCEDURE [dbo].[s_Ins_ImportData]
GO
...
-- 此文档是开发环境使用，其他环境需要格式化之后使用
-- 注意事项：
-- 1. 每写一个模块，就需要添加一行“--”，用于格式化代码，防止字符串超长
-- 2. 修改传入参数时参考【说明1】
-- 3. 
-- ============================================= 
CREATE PROCEDURE [dbo].[s_Ins_ImportData]
	@G2BSDB nvarchar(100),
	@G2BFDB nvarchar(100),
	@LiqDB nvarchar(100),
...
DECLARE @sql30 NVARCHAR(4000)
  SET @sql1 = N'	
	...
	-- 传入的参数只在这里使用，方便StringFormat【说明1】
...
BEGIN TRAN tran_importdata --开始事务
...
'
 SET @sql4 = N'	IF OBJECT_ID(N''TEMPDB..#hol1_tmp'',N''U'') IS NOT NULL			DROP TABLE  #hol1_tmp;
	...
	-- 表 stcontrol --> Liq.stcontrol 日期和开关（原有）
	...
	-- #temp_view_er_exchange_rate  货币兑换率
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	-- #temp_view_er_client_fund_movement 客户资金流动金额（已换算）
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	--SET
	--	orig_amount = floor(e.amount), -- 跟EBS3向下取整的处理
	...
	-- 跟EBS3错误的结果（把数据Update成第1条记录）
	UPDATE
		#temp_view_er_client_fund_movement
	SET
		orig_amount = floor(tb.amount), -- 跟EBS3向下取整的处理
	...
	-- #temp_view_er_client_portfolio  客户有价证券收市价（已换算）
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
...
'
 SET @sql5 = N'	--	#temp_view_er_client_portfolio
	--SET
	--	orig_closing_price = floor(e.closing_price), -- 跟EBS3向下取整的处理
	...
	SET
		orig_closing_price = floor(tb.closing_price), -- 跟EBS3向下取整的处理
	...
	-- #temp_view_er_client_trade_namt_with_fee  客户 净总值（已换算）
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	SET
		orig_price = floor(e.price), -- 跟EBS3向下取整的处理
	...
	-- #temp_view_er_client_master_bals 客户 余额
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	-- #temp_view_er_client_master_mkt_mrg_value  客户 市场 利润 价值
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	-- #temp_view_er_client_master  客户 余额、市场 利润（合并上面两个表），再换算
'
...
'
 SET @sql7 = N'		credit_lmt = a.credit_lmt,
	...
	-- #temp_ibscaccupl_view 客户 现金 流动 的信息
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	-- #temp_ststktxnlst_view 客户 净总值（未换算）
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
...
'
 SET @sql9 = N'		) AS sum_qty,
		...
		CONVERT(NUMERIC(16, 4), 
			...
			* 
			P.closing_price -- closePrice的小数位是6位，这里跟EBS3转成4位
		) AS coll_mkt,
		...
		) AS coll_mar,	-- EBS3为12位小数（4+4+4），这里用4位
	...
	---------------------------------------------------------------------------------------------------------------------------------------------------
		--备注：
		--1. 问题：表stcontrol 和 ESL_Liq的 stcontrol 重复（使用Liq）;
		...
		--staemaster		--> Liq.staemaster（原有） ? Import只导2列，但原本数据有5列，是否正确？
	...
	-- 表 Ibssttxndate --> ibs_st_txn_date 设置日期
	SET @currentStep = @currentStep + 1
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	-- 表 Testbal --> Liq.testbal  AE 金额（原有）
	SET @currentStep = @currentStep + 1
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
		N''Save data - testbal  ...'';
...
'
 SET @sql11 = N'	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	-- 表 Ibscaccup1 --> ibs_cac_cup 客户 金额
	SET @currentStep = @currentStep + 1
	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
...
'
 SET @sql12 = N'	SET @currentStep = @currentStep + 1
	...
	SET @sdbdateBegin = DATEADD(m, DATEDIFF(m, 0, @sdbdate), 0) -- 这个月1号
	SET @sdbdateEnd = DATEADD(m, 1, DATEADD(m, DATEDIFF(m, 0, @sdbdate), 0)) --下个月1号;
	...
	BEGIN
		...
		SET @lastmonthBegin = DATEADD(m, DATEDIFF(m, 0, @lastmonth), 0) -- 这个月1号
		SET @lastmonthEnd = DATEADD(m, 1, DATEADD(m, DATEDIFF(m, 0, @lastmonth), 0)) --下个月1号;
...
'
 SET @sql13 = N'					AND (
	...
	SET @fdbdateBegin = DATEADD(m, DATEDIFF(m, 0, @fdbdate), 0); -- 这个月1号
	SET @fdbdateEnd = DATEADD(m, 1, DATEADD(m, DATEDIFF(m, 0, @fdbdate), 0)); --下个月1号;
	...
	BEGIN
		...
		-- [fixed]ERROR: Violation of UNIQUE KEY constraint ''uq_exchangerate''. Cannot insert duplicate key in object ''dbo.exchangerate''.
		-- 添加数据到 exchangerate
		INSERT INTO
			exchangerate
		(
		...
		SELECT
			...
			a.ex_rate,
			@fdbdate,
			DATEADD(m, DATEDIFF(m, 0, @fdbdate), 0)
			-- 当月1号
		FROM
			exchangerate a
			INNER JOIN (
				...
				GROUP BY
					currency_in
'
 SET @sql14 = N'			) b ON a.currency_in = b.currency_in
			AND a.tdate = b.mmth;
	...
	BEGIN;
		...
		-- 删除 ' + @LiqDB + N'.dbo.monthcommdetailf 上个月数据
		DELETE FROM
		...
		-- 添加 ' + @LiqDB + N'.dbo.monthcommdetailf 上个月数据
		INSERT INTO
			' + @LiqDB + N'.dbo.monthcommdetailf
			(
				 [accno]
		...
		-- 删除 ' + @LiqDB + N'.dbo.monthcommf 上个月数据
		DELETE FROM
		...
		-- 添加 ' + @LiqDB + N'.dbo.monthcommf 上个月数据
		INSERT INTO
			' + @LiqDB + N'.dbo.monthcommf
			(
				 [ACCNO]
	...
	-- 删除 ' + @LiqDB + N'.dbo.monthcommdetailf 当月数据	
	DELETE FROM
		' + @LiqDB + N'.dbo.monthcommdetailf
	WHERE
	...
	-- 添加 ' + @LiqDB + N'.dbo.monthcommdetailf 当月数据
	INSERT INTO
		' + @LiqDB + N'.dbo.monthcommdetailf
		(
			[accno]
...
'
 SET @sql15 = N'		--top 100 -- Debug test code
	...
	-- 删除 ' + @LiqDB + N'.dbo.monthcommf 当月数据
	DELETE FROM
		' + @LiqDB + N'.dbo.monthcommf
	WHERE
		YEAR(mth) = YEAR(@fdbdate)
	...
	-- 添加 ' + @LiqDB + N'.dbo.monthcommf 当月数据
	INSERT INTO
		' + @LiqDB + N'.dbo.monthcommf
		(
			 [ACCNO]
...
'
 SET @sql16 = N'		N''Save data - Bal.acbal  ...'';
	...
	SELECT
		...
		(underreg_qty + onhand) * closing_price * 1 AS market_value -- TODO：EBS3语法写错，先用*1代替，debug
	INTO #stockcon
	FROM
	...
	*/;
	...
	SET
		@lcDate = DATEADD(d, DATEDIFF(d, 0, @sdbdate), 0)-- 去掉时间部分;

	-- 删除 stockconcentration 当天数据
	DELETE FROM
...
'
 SET @sql17 = N'	EXEC [s_Ins_ImportDataHistory] @tempgroup, @tempuser, @currentStep, @totleStep,
	...
	/*  TODO: debug:EBS3执行出错，没有运行（前面生成#stockcon出错）
	---- 添加 stockconcentration 当天数据 
	--INSERT INTO
	--	' + @BalanceDB + N'.dbo.stockconcentration(
	--		accno,
	--		tdate,
...
IF(@tran_error > 0)
BEGIN
	ROLLBACK TRAN tran_importdata; --执行出错，回滚事务(指定事务名称)
END 
ELSE
BEGIN
	COMMIT TRAN tran_importdata; --没有异常，提交事务(指定事务名称)
	-- 返回正常值
	SELECT 
		0 AS [error],
		'''' AS [message];
